...
import { getUnifiedPatchPreprocessor } from '@/preprocessing';
import { getConsistencyValidator } from '@/output/anthropic/consistency-validator';
...
export class UnifiedConversionOpenAIClient implements Provider {
  ...
  private async applyPostprocessing(rawResponse: any, context: ConversionContext): Promise<BaseResponse> {
    ...
    try {
      // 首先通过预处理器处理响应
      const preprocessedResponse = await this.preprocessor.preprocessResponse(
        rawResponse,
        'openai-compatible' as any,
        rawResponse.model || 'unknown',
        context.requestId
      );

      // 转换为Anthropic格式
      const anthropicResponse = this.convertToAnthropicResponse(preprocessedResponse, context);
      
      // 应用一致性验证器 - transformer后的处理层guard
      const { response: validatedResponse, result } = this.consistencyValidator.validateAndFix(
        anthropicResponse,
        context.requestId
      );

      this.recordTransformationStep(context, 'postprocessing', {
        consistencyCheck: {
          consistent: result.consistent,
          fixed: result.fixed,
          originalStopReason: result.originalStopReason,
   ...
   * 🎯 统一finish reason记录点
   */
  private recordUnifiedFinishReason(response: BaseResponse, context: ConversionContext): void {
    const stepStart = Date.now();
