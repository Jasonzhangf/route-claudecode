...
import { ServerFactory } from './modules/server/src';
import { startupService, StartupConfig } from './modules/bootstrap/src';
...
class PortManager {
   ...
   * @returns 进程信息数组
   */
  static async getPortProcesses(port: number): Promise<Array<{pid: string, command: string}>> {
    try {
      const result = await execAsync(`lsof -i :${port}`);
   ...
   * @returns 是否成功清理端口
   */
  static async checkAndClearPort(port: number): Promise<boolean> {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const startupResult = await startupService.start(startupConfig);
      ...
      if (startupResult.pipelines) {
        ...
        if (pipelineSummary.allPipelines) {
          pipelineSummary.allPipelines.forEach((pipeline: any) => {
            if (pipeline.provider) providers.add(pipeline.provider);
            if (pipeline.model) models.add(pipeline.model);
          });
        }
