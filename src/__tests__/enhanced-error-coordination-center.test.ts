...
import { EnhancedErrorCoordinationCenter } from '../core/enhanced-error-coordination-center';
import { EnhancedErrorCoordinationCenterFactory } from '../core/enhanced-error-coordination-center-factory';
import { ErrorContext, ErrorHandlingResult, CustomErrorHandlingStrategy } from '../core/enhanced-error-coordination-center';
 ...
 * 测试用的自定义错误处理策略
 */
class TestCustomStrategy implements CustomErrorHandlingStrategy {
  shouldHandle(error: Error, context: ErrorContext): boolean {
  ...
  async handle(error: Error, context: ErrorContext): Promise<ErrorHandlingResult> {
    return {
      ...
      returnedError: {
        message: 'Handled by custom strategy',
        custom: true
      },
      context
    };
 ...
 * 增强版错误处理协调中心测试类
 */
export class EnhancedErrorCoordinationCenterTest {
  private errorCoordinationCenter: EnhancedErrorCoordinationCenter;
  ...
  constructor() {
    ...
    this.errorCoordinationCenter = EnhancedErrorCoordinationCenterFactory.createForTesting();
  }
  ...
  async runAllTests(): Promise<void> {
    ...
    try {
      ...
      await this.testCustomStrategies();
  ...
  private async testPipelineDestruction(): Promise<void> {
    ...
    const centerWithDestruction = EnhancedErrorCoordinationCenterFactory.create({
   ...
   * 测试自定义策略
   */
  private async testCustomStrategies(): Promise<void> {
    console.log('🧪 测试自定义策略...');
    
    const customStrategy = new TestCustomStrategy();
    this.errorCoordinationCenter.addCustomStrategy(customStrategy);
    
    const customError = new Error('This is a custom error message');
    const context: ErrorContext = {
      requestId: 'test-custom-1',
      pipelineId: 'pipeline-1',
      attemptNumber: 0,
      maxAttempts: 3,
      isLastAttempt: false,
    ...
    const result = await this.errorCoordinationCenter.handleError(customError, context);
    
    if (\!result.success) {
      throw new Error('自定义策略处理应该成功');
    }
    
    if (result.returnedError && \!result.returnedError.custom) {
      throw new Error('应该由自定义策略处理');
    }
    
    // 移除自定义策略
    this.errorCoordinationCenter.removeCustomStrategy(customStrategy);
    
    console.log('✅ 自定义策略测试通过');
  }
  ...
  private async testStatistics(): Promise<void> {
    ...
    if (stats.avgHandlingTime <= 0) {
      throw new Error('平均处理时间应该大于0');
    }
    
    if (stats.successRate < 0 || stats.successRate > 1) {
      throw new Error('成功率应该在0到1之间');
    }
   ...
   * 测试错误历史记录
   */
  private async testErrorHistory(): Promise<void> {
    console.log('🧪 测试错误历史记录...');
    
    const error = new NetworkError('Connection refused');
    const context: ErrorContext = {
      requestId: 'test-history-1',
      pipelineId: 'pipeline-1',
      attemptNumber: 0,
      maxAttempts: 3,
      isLastAttempt: false,
    ...
    const history = this.errorCoordinationCenter.getErrorHistory(10);
    
    if (history.length < 1) {
      throw new Error('错误历史记录不正确');
    }
    
    if (\!history[0].error || \!history[0].context || \!history[0].result) {
      throw new Error('错误历史记录格式不正确');
    }
    
    console.log('✅ 错误历史记录测试通过');
  }
}

// 运行测试
if (require.main === module) {
  const test = new EnhancedErrorCoordinationCenterTest();
