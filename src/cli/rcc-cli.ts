...
import { ApplicationBootstrap, ApplicationRuntime, BootstrapConfig } from '../bootstrap/application-bootstrap';
...
export class RCCCli implements CLICommands {
  ...
  private async stopServer(options: StopOptions): Promise<void> {
    ...
    try {
      ...
      // 清理全局实例
      (global as any).applicationRuntime = undefined;
  ...
  private async getServerStatus(options: StatusOptions): Promise<ServerStatus> {
    ...
    return {
      ...
      health: {
        ...
        checks: [
          ...
          },
          {
            name: 'Layer Health',
            status:
              appStats && (appStats as any).requestProcessorStats?.layerHealth && Object.keys((appStats as any).requestProcessorStats.layerHealth).length > 0
                ? 'pass'
                : 'warn',
            responseTime: 2,
          },
      ...
      pipeline: {
        ...
        layerHealth: (appStats as any)?.requestProcessorStats?.layerHealth,
      },
    };
  }
  ...
  private setupPipelineEventListeners(): void {
    ...
    secureLogger.info('Application runtime event listeners setup completed');
   ...
   */
  private async _performApplicationBootstrap(options: StartOptions): Promise<{success: boolean; runtime?: ApplicationRuntime; errors: string[]}> {
    try {
      const { BOOTSTRAP_CONFIG, SCHEDULER_DEFAULTS } = await import('../constants/bootstrap-constants');
      const { ApplicationBootstrap } = await import('../bootstrap/application-bootstrap');
      const { LoadBalanceStrategy } = await import('../interfaces/scheduler/dynamic-scheduler');
      ...
      const bootstrapConfig = {
        ...
        scheduler: {
          strategy: SCHEDULER_DEFAULTS.STRATEGY as any,
  ...
  private setupApplicationEventListeners(): void {
    ...
    secureLogger.info('🔊 [RCC CLI] 应用程序事件监听器已设置', {
