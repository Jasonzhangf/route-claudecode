...
export class CommandParser implements CLIHandler {
   ...
   * 解析选项和参数
   */
  private parseOptions(commandDef: CommandDefinition, args: string[]): {
   ...
   * 执行启动命令
   */
  private async executeStartCommand(command: ParsedCommand): Promise<void> {
   ...
   * 执行停止命令
   */
  private async executeStopCommand(command: ParsedCommand): Promise<void> {
   ...
   * 执行客户端命令
   */
  private async executeCodeCommand(command: ParsedCommand): Promise<void> {
   ...
   * 执行状态命令
   */
  private async executeStatusCommand(command: ParsedCommand): Promise<void> {
