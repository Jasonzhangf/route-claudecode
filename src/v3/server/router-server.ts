...
export class RouterServer {
  ...
  private async handleMessagesRequest(request: FastifyRequest, reply: FastifyReply): Promise<any> {
    ...
    // Record request input with debug system
    if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
      this.debugSystem.debugComponents.recorder.recordLayerIO('client', 'input', {
        requestId,
        timestamp: new Date().toISOString(),
        headers: request.headers,
        body: request.body,
    ...
    try {
      ...
      try {
        ...
        // Record router layer output with debug system
        if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
          this.debugSystem.debugComponents.recorder.recordLayerIO('router', 'output', {
            providerId,
      ...
      if (baseRequest.stream) {
        ...
        // Record provider-protocol layer input
        if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
      ...
      // Record transformer layer input
      if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
        this.debugSystem.debugComponents.recorder.recordLayerIO('transformer', 'input', {
          providerResponse: pipelineContext,
          pipelineContext
        }, {
          requestId,
          processingTime: Date.now() - startTime,
      ...
      // Record transformer layer output
      if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
        this.debugSystem.debugComponents.recorder.recordLayerIO('transformer', 'output', {
          pipelineResponse,
          transformationApplied: pipelineResponse !== providerResponse
        }, {
          requestId,
          processingTime: Date.now() - startTime,
          layer: 'transformer'
      ...
      // Record post-processor layer input
      if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
        this.debugSystem.debugComponents.recorder.recordLayerIO('post-processor', 'input', {
          pipelineResponse,
          baseRequest
        }, {
          requestId,
      ...
      if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
        this.debugSystem.debugComponents.recorder.recordLayerIO('post-processor', 'output', {
          finalResponse
        }, {
          requestId,
          processingTime: Date.now() - startTime,
      ...
      // Record server layer output (final response to client)
      if (this.debugSystem && this.debugSystem.debugComponents?.recorder) {
        this.debugSystem.debugComponents.recorder.recordLayerIO('server', 'output', {
  ...
  private async handleStreamingRequest(
    ...
    try {
      ...
      while (!isDone) {
        ...
        try {
          // 🔧 应用响应流水线处理（如果chunk.data存在）
          if (currentChunk.data) {
            const processedData = await this.responsePipeline.process(currentChunk.data, pipelineContext);
            processedChunk = { ...currentChunk, data: processedData };
          } else {
            processedChunk = currentChunk;
          }
          
          this.logger.debug('Streaming chunk processed through pipeline', {
            originalChunk: currentChunk.event,
            pipelineApplied: !!currentChunk.data,
            finalEvent: processedChunk.event
          }, requestId, 'streaming-pipeline');
          
        } catch (error) {
          this.logger.warn('Pipeline processing failed for streaming chunk, using original', {
            error: error instanceof Error ? error.message : String(error),
        ...
        if (processedChunk.event === 'message_delta' && processedChunk.data?.delta?.stop_reason) {
          ...
          if (isToolUse) {
            ...
            logFinishReasonDebug(stopReason, {
              requestId,
              provider: provider.name,
              model: request.model,
              port: this.config.server.port,
