...
import { PipelineRouter, RoutingTable } from '../router/pipeline-router';
...
export class PipelineLifecycleManager extends EventEmitter {
  ...
  private routingTable?: RoutingTable; // 保存从配置生成的路由表
  ...
  private async initializeAndStartServer(): Promise<void> {
    try {
      ...
      const serverConfig: PipelineServerConfig = {
        ...
        timeout: 30000,
        // 🔧 关键修复：移除硬编码路由规则，完全依赖用户配置和虚拟模型映射
        // 路由规则现在完全来自用户配置文件和runtime routing table
      };
      ...
      // 设置路由器实例（必须已初始化）
      if (!this.router) {
        throw new Error('Router must be initialized before creating PipelineServer');
      }
      this.pipelineServer.setRouter(this.router);
      secureLogger.info('Router successfully injected into PipelineServer', {
        routerReady: !!this.router,
        routerStats: this.router.getStatistics ? this.router.getStatistics() : 'no stats available'
      });
  ...
  private async processServerCompatibilityLayer(
    ...
    // 🔧 关键修复：完全基于配置的动态模块加载，无硬编码
    try {
      // 根据系统配置动态加载兼容性模块
      const compatibilityModule = await this.loadCompatibilityModule(compatibilityTag, moduleInfo, request, context, routingDecision);
      ...
      if (compatibilityModule) {
        ...
        // 🔧 关键修复：确保__internal对象在兼容性处理后得到保留
        if (request.__internal) {
          processedRequest.__internal = request.__internal;
          secureLogger.debug('Preserved __internal config through compatibility layer', {
            requestId: context.requestId,
            hasInternalConfig: !!processedRequest.__internal,
            endpoint: processedRequest.__internal.endpoint,
          });
        }
        
        secureLogger.info('Server compatibility processing completed', {
          requestId: context.requestId,
          compatibilityTag,
          moduleType: moduleInfo?.module,
        ...
        // 如果无法加载特定模块，使用通用兼容性处理
        return this.processGenericCompatibility(request, context);
      }
    } catch (error) {
      secureLogger.error('Server compatibility processing failed - ZERO FALLBACK POLICY', {
        requestId: context.requestId,
        compatibilityTag,
        moduleType: moduleInfo?.module,
        error: error.message,
      ...
      throw new Error(`Server compatibility processing failed for ${compatibilityTag}: ${error.message}`);
    }
  }

  /**
   * 动态加载兼容性模块 - 完全基于配置，无硬编码
   */
  private async loadCompatibilityModule(compatibilityTag: string, moduleInfo: any, request: any, context: RequestContext, routingDecision?: any): Promise<any | null> {
    if (!moduleInfo || !moduleInfo.module) {
      secureLogger.debug('No specific compatibility module configured, will use generic', {
        requestId: context.requestId,
        compatibilityTag,
      });
      return null;
    ...
    try {
      secureLogger.debug('Loading compatibility module', {
        requestId: context.requestId,
        compatibilityTag,
        moduleType: moduleInfo.module,
      });

      // 🔧 关键修复：动态模块路径映射，基于系统配置
      const modulePathMap: Record<string, string> = {
        'LMStudioCompatibilityModule': '../modules/pipeline-modules/server-compatibility/lmstudio-compatibility',
        'AnthropicCompatibilityModule': '../modules/pipeline-modules/server-compatibility/anthropic-compatibility',
        'OllamaCompatibilityModule': '../modules/pipeline-modules/server-compatibility/ollama-compatibility',
        'GenericCompatibilityModule': '../modules/pipeline-modules/server-compatibility/passthrough-compatibility',
      };

      const modulePath = modulePathMap[moduleInfo.module];
      if (!modulePath) {
        secureLogger.warn('Unknown compatibility module type', {
          requestId: context.requestId,
          moduleType: moduleInfo.module,
          availableModules: Object.keys(modulePathMap),
        });
        return null;
      }
      ...
      const ModuleClass = moduleExports[moduleInfo.module];
      
      if (!ModuleClass) {
        secureLogger.error('Compatibility module class not found in exports', {
          requestId: context.requestId,
          moduleType: moduleInfo.module,
          modulePath,
          availableExports: Object.keys(moduleExports),
        });
        return null;
      }

      // 从pipeline ID和request构建模块配置
      const firstPipelineId = routingDecision?.availablePipelines?.[0];
      const targetModel = this.extractModelFromPipelineId(firstPipelineId);
      
      // 🔧 关键修复：通用配置构建，适用于所有兼容性模块
      const moduleConfig = {
        baseUrl: request.__internal?.endpoint,
        apiKey: request.__internal?.apiKey,
        timeout: request.__internal?.timeout || 30000,
        maxRetries: request.__internal?.maxRetries || 3,
        retryDelay: 1000,
        models: [targetModel],
        maxTokens: {},
        compatibilityTag,
      ...
      const compatibilityModule = new ModuleClass(moduleConfig);
      
      secureLogger.info('Compatibility module loaded successfully', {
        requestId: context.requestId,
        compatibilityTag,
        moduleType: moduleInfo.module,
        targetModel,
        hasEndpoint: !!moduleConfig.baseUrl,
      });

      return compatibilityModule;
      
    } catch (error) {
      secureLogger.error('Failed to load compatibility module', {
        requestId: context.requestId,
        compatibilityTag,
        moduleType: moduleInfo?.module,
        error: error.message,
  ...
  /**
   * 初始化路由器
   * 使用运行时从配置文件生成的路由表创建PipelineRouter
   */
  private async initializeRouter(): Promise<void> {
    try {
      if (!this.routingTable) {
        throw new Error('Routing table not available - must be created before router initialization');
      }

      secureLogger.info('Initializing router from runtime routing table', {
        routeCount: Object.keys(this.routingTable.routes).length,
        defaultRoute: this.routingTable.defaultRoute,
        serverPort: this.config.server.port
      });

      // 使用运行时生成的路由表创建路由器
      this.router = PipelineRouter.fromRoutingTable(this.routingTable);
      
      secureLogger.info('Router initialized successfully from runtime configuration', {
        routeCount: this.router.getStatistics().totalRoutes,
        routingTableSource: 'runtime-config'
      });

    } catch (error) {
      secureLogger.error('Failed to initialize router from runtime routing table', {
        error: error.message,
        stack: error.stack,
        hasRoutingTable: !!this.routingTable
      });
  ...
  /**
   * 从配置中提取配置名称
   * 🔧 关键修复：通用配置命名，基于用户配置而非硬编码
   */
  private extractConfigNameFromConfig(): string {
    const port = this.config.server.port;
    
    // 从用户配置中获取provider类型，生成通用命名
    const providers = this.config.providers || [];
    const primaryProvider = providers[0]?.name || 'generic';
    
    return `rcc-v4-${primaryProvider}-${port}`;
  }
