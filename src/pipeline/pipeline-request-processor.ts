...
export class PipelineRequestProcessor extends EventEmitter {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (statusCode && statusCode >= 400 && statusCode < 500) {
          ...
          // 返回API错误而不是抛出异常
          return this.createApiErrorResponse(error, statusCode, context.requestId);
        }
        
        // 检查是否应该重试
        const shouldRetry = this.shouldRetryError(error, statusCode);
   ...
   */
  private shouldRetryError(error: Error, statusCode?: number): boolean {
    // 超时错误 - 可重试
   ...
   */
  private createApiErrorResponse(error: any, statusCode?: number, requestId?: string): any {
