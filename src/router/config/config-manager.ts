...
export class RouterConfigManager extends EventEmitter {
  ...
  async loadConfig(globalConfig: RCCv4Config): Promise<RouterConfig> {
    try {
      ...
      const routerConfig: RouterConfig = {
        routingStrategies: this.convertRoutingStrategies(globalConfig.routing.routingStrategies || {}),
        loadBalancing: {
          algorithm: 'weighted',
   ...
   * 转换路由策略配置
   */
  private convertRoutingStrategies(strategies: Record<string, any>): Record<string, RoutingStrategyConfig> {
    const converted: Record<string, RoutingStrategyConfig> = {};
