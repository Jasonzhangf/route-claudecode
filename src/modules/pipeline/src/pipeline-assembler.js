 ...
 * 默认模块选择策略
 */
class DefaultModuleSelectionStrategy {
    constructor(registry) {
        this.registry = registry;
    }
    async selectModule(type, config) {
        ...
        if (provider) {
            // 优先选择provider特定的模块
            const specificModule = availableModules.find(module => module.getName().toLowerCase().includes(provider.toLowerCase()));
            if (specificModule) {
    ...
    async validateModuleCompatibility(modules) {
        // 基础兼容性检查
...
class PipelineAssembler {
     ...
     * @returns 组装结果
     */
    async assemble(pipelineConfigs) {
        if (this.isDestroyed) {
     ...
     * 组装单个流水线
     */
    async _assembleSinglePipeline(config) {
        const startTime = Date.now();
        const errors = [];
        const pipeline = {
        ...
        for (const layerConfig of config.layers) {
            const moduleType = layerConfig.type;
     ...
     * 按路由模型分组流水线
     */
    _groupPipelinesByRouteModel(pipelines) {
        const grouped = {};
     ...
     * 验证所有连接
     */
    async _validateAllConnections(pipelines) {
        for (const pipeline of pipelines) {
     ...
     * 执行健康检查
     */
    async _performHealthChecks(pipelines) {
        const healthChecks = [];
        for (const pipeline of pipelines) {
            for (const module of pipeline.modules) {
                const startTime = Date.now();
     ...
     * 从routeId提取routeName
     */
    _extractRouteNameFromRouteId(routeId) {
     ...
     * 创建失败结果
     */
    _createFailureResult(errors, warnings, startTime) {
        return {
            success: false,
            pipelinesByRouteModel: {},
            allPipelines: [],
