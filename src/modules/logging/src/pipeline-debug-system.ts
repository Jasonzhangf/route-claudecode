...
export class PipelineDebugSystem extends EventEmitter {
   ...
   * 执行完整的初始化检查
   */
  async performInitializationCheck(routingTable: any): Promise<ValidationResult> {
   ...
   * 根据路由表计算期望的流水线
   */
  calculateExpectedPipelines(routingTable: any): ExpectedPipeline[] {
    ...
    if (!routingTable || !routingTable.routes) {
      secureLogger.warn('Routing table is invalid or missing routes');
      return expected;
    }
    ...
    for (const [virtualModel, routes] of Object.entries(routingTable.routes)) {
      ...
      for (const route of routes) {
        if (!route || !route.pipelineId) continue;
        ...
        const providerModel = `${route.provider || 'unknown'}-${targetModel}`;
        ...
        if (!seenProviderModels.has(providerModel)) {
          ...
          const expectedPipeline: ExpectedPipeline = {
            ...
            apiKeyIndex: route.apiKeyIndex || 0,
            shouldExist: true
          };
