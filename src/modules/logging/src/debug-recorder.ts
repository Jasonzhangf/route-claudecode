...
export interface DebugRecorder {
  ...
  recordPipelineExecution(requestId: string, pipeline: any, data: unknown): void;
  recordModuleInput(moduleName: string, requestId: string, input: unknown): void;
...
export class DebugRecorderImpl extends EventEmitter implements DebugRecorder {
  ...
  recordModuleError(moduleName: string, requestId: string, error: RCCError): void {
    try {
      ...
      if (moduleRecords.length > 0) {
        const lastRecord = moduleRecords[moduleRecords.length - 1];
        lastRecord.error = filteredError as any;
        lastRecord.endTime = now;
        lastRecord.endTimeReadable = this.formatReadableTime(now);
        lastRecord.duration = now - lastRecord.startTime;
      }
      ...
      this.emit('module-error-recorded', {
        moduleName,
        requestId,
        error: (filteredError as any).message || String(filteredError),
        timestamp: now,
      });
    } catch (recordError) {
  ...
  async saveRecord(record: DebugRecord): Promise<void> {
    try {
      ...
      const filteredRecord: DebugRecord = {
        ...
        response: record.response ? {
          ...
          body: record.response.body || null,
          duration: record.response.duration || 0
        } : undefined,
        error: record.error ? {
          type: record.error.type || 'unknown',
          message: record.error.message || '',
          module: record.error.module || '',
          stack: record.error.stack || ''
        } : undefined,
        pipeline: {
          id: record.pipeline.id,
          provider: record.pipeline.provider,
          model: record.pipeline.model,
          modules: record.pipeline.modules.map(module => ({
            moduleName: module.moduleName,
            startTime: module.startTime,
            startTimeReadable: module.startTimeReadable,
            endTime: module.endTime,
            endTimeReadable: module.endTimeReadable,
            ...
            error: module.error ? (this.filter ? this.filter.filterError(module.error).filtered : module.error) as RCCError : undefined,
  ...
  private createBaseRecord(requestId: string, data: any): Partial<DebugRecord> {
    ...
    return {
      ...
      port: port,
      sessionId: this.findCurrentSessionId(port),
      request: {
