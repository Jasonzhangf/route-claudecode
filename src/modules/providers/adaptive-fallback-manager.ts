...
export class AdaptiveFallbackManager {
  ...
  private circuitBreakerStatus: Map<string, {
    isOpen: boolean;
    failureCount: number;
   ...
   * 选择最佳降级模型
   */
  private async selectBestFallbackModel(primaryModel: string, priority: 'high' | 'normal' | 'low'): Promise<string | null> {
  ...
  private predictUpcoming429(modelName: string): boolean {
    ...
    if (recent429s.length >= 3) {
      ...
      if (intervals.length >= 2) {
        const isDecreasing = intervals.every((interval, index) => 
          index === 0 || interval <= intervals[index - 1] * 1.2);
  ...
  private applyAdaptiveCooldown(modelName: string): void {
    ...
    // 计算冷却级别 (基于连续失败次数)
    health.adaptiveCooldownLevel = Math.min(
      health.consecutiveFailures,
      this.MAX_COOLDOWN_LEVELS
    );
  ...
  private triggerCircuitBreaker(modelName: string): void {
    ...
    const backoffMs = Math.min(
      60000 * Math.pow(2, circuitBreaker.failureCount - 1),
      30 * 60 * 1000
    );
  ...
  updateModelHealth(
    ...
    if (isSuccess) {
      ...
      if (responseTime) {
        health.avgResponseTime = (health.avgResponseTime * (health.totalRequests - 1) + responseTime) / health.totalRequests;
      }
    } else {
      health.consecutiveFailures++;
  ...
  private calculateHealthScore(health: ModelHealthStatus): number {
    ...
    const recent429Count = health.failurePattern.recentFailures
      .filter(f => f.errorType === '429')
      .filter(f => (Date.now() - f.timestamp.getTime()) < 15 * 60 * 1000) // 15分钟内
      .length;
    score -= Math.min(recent429Count * 5, 20);
    
    // 响应时间影响 (权重10%)
    if (health.avgResponseTime > 5000) { // 超过5秒
