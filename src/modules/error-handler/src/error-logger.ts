...
interface ErrorLogEntry {
  timestamp: string;
  requestId: string;
  errorType: string; // 使用string类型以兼容不同的ErrorType枚举
  errorMessage: string;
  errorStack?: string;
  pipelineId?: string;
  layerName?: string;
...
export class ErrorLogger {
  ...
  async logError(error: Error, context: ErrorContext, classification: ErrorClassification): Promise<void> {
    try {
      ...
      const logEntry: ErrorLogEntry = {
        ...
        errorType: classification.type as string, // 强制转换为string类型
        ...
        isRetryable: (classification as any).isRetryable,
        isFatal: (classification as any).isFatal,
        httpStatusCode: this.getHttpStatusCodeForErrorType(classification.type as any),
      ...
      if (this.config.enableAuditLogging) {
        secureLogger.audit('ERROR_OCCURRED', {
          requestId: context.requestId,
          errorType: classification.type as string,
          errorMessage: error.message,
          pipelineId: context.pipelineId,
          isRetryable: (classification as any).isRetryable,
          isFatal: (classification as any).isFatal,
   ...
   * @param context 错误上下文
   */
  logHandlingResult(result: unknown, context: ErrorContext): void {
    try {
      secureLogger.info('Error handling completed', {
        requestId: context.requestId,
        actionTaken: (result as any).actionTaken,
