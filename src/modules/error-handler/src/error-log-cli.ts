...
program
  ...
  .action(async (options) => {
    ...
    cliOutput('\n📋 错误摘要报告');
    cliOutput('================');
    cliOutput(`报告ID: ${report.reportId || 'N/A'}`);
    cliOutput(`时间范围: ${new Date(report.timeRange?.startTime || Date.now()).toLocaleString()} - ${new Date(report.timeRange?.endTime || Date.now()).toLocaleString()}`);
    ...
    // 显示TOP错误
    if ((report as any).statistics.topErrors && (report as any).statistics.topErrors.length > 0) {
      cliOutput('\n🔥 高频错误 (TOP 5):');
      (report as any).statistics.topErrors.slice(0, 5).forEach((error: any, index: number) => {
        cliOutput(`  ${index + 1}. ${error.message.substring(0, 60)}... (${error.count}次)`);
      });
    }

    // 显示问题流水线
    if ((report as any).statistics.problemPipelines && (report as any).statistics.problemPipelines.length > 0) {
      cliOutput('\n⚠️  问题流水线:');
      (report as any).statistics.problemPipelines.forEach((pipeline: any) => {
        cliOutput(`  🔧 ${pipeline.pipelineId}: ${pipeline.errorCount}错误 (${(pipeline.errorRate * 100).toFixed(1)}%)`);
      });
    }

    // 显示建议
    if ((report as any).recommendations && (report as any).recommendations.length > 0) {
      cliOutput('\n💡 处理建议:');
      (report as any).recommendations.forEach((rec: any, index: number) => {
