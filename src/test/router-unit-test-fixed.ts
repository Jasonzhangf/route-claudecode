...
function createPipelineConfiguration(
  ...
  return {
    ...
    apiKeyIndex: keyIndex,
    modules: {
      // 修正ClientModule配置结构 - 确保config字段正确传递
      client: {
        type: 'ClientModule',
        config: {
          protocol: 'anthropic'
      ...
      // 修正ServerCompatibility模块配置结构 - 使用通用类型
      serverCompatibility: {
