...
# Use direct esbuild instead of npm run build to avoid TypeScript errors
rm -rf dist/
echo "📦 Installing dependencies..."
npm ci
echo "🔨 Building with esbuild directly..."
esbuild src/cli.ts --bundle --platform=node --outfile=dist/cli.js --external:tiktoken --external:@anthropic-ai/sdk --external:@google/genai --banner:js="#!/usr/bin/env node"
esbuild src/cli/dynamic-model-cli.ts --bundle --platform=node --outfile=dist/dynamic-model-cli.js --external:tiktoken --external:@anthropic-ai/sdk --external:@google/genai --banner:js="#!/usr/bin/env node"
chmod +x dist/cli.js
