...
class ConfigLoader {
     ...
     * 加载并合并配置
     */
    async loadConfig(command, options = {}) {
        const configs = {};
        // 1. 加载默认配置
     ...
     * 合并配置
     */
    mergeConfig(configs, newConfig, source, priority) {
     ...
     * 导出配置为环境变量格式
     */
    exportAsEnvVars(config, prefix = 'RCC') {
        const envVars = [];
        for (const [key, value] of Object.entries(config)) {
            const definition = this.schema[key];
