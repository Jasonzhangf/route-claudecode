...
// Test script for Gemini models availability through Anthropic API endpoint
console.log('🧪 Starting Gemini Models Availability Test via Anthropic API');
...
// Simple test request template (Anthropic format)
...
function testModel(modelName) {
  ...
  // Using curl to send the request to Anthropic API endpoint
  const curlProcess = spawn('curl', [
    '-X', 'POST',
    'http://localhost:3456/v1/messages',
    '-H', 'Content-Type: application/json',
    '-H', 'Authorization: Bearer test-key',
    '-H', 'User-Agent: Claude-Code-Router-Test',
    '-d', `@${tempRequestFile}`,
  ...
  curlProcess.on('close', (code) => {
    ...
    if (response) {
      try {
        ...
        console.log(`❌ ${modelName} - FAILED - Could not parse response:`, response.substring(0, 200));
...
function finishTest() {
  ...
  // Generate configuration for working models
  if (workingModels.length > 0) {
    console.log('\n🔧 CONFIGURATION FOR WORKING MODELS:');
    console.log('\nAdd the following to your routing configuration:');
    console.log(`
"gemini": {`);
    console.log(`  "providers": [`);
    console.log(`    {`);
    console.log(`      "provider": "gemini-jason-gmail",`);
